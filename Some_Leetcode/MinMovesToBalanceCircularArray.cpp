...
public:
    long long minMoves(vector<int>& balance) {
        ...
        long long count = 0;
        while (balance[index] < 0) {
            if (abs(balance[index])>(balance[left])){
                balance[index]+= balance[left];
                count+=(balance[left]*distance);
            }
            else{
                count+=(abs(balance[index])*distance);
                balance[index]+= balance[left];
            }      
            if (balance[index]>=0) return count;
            if (abs(balance[index])>(balance[right])){
                balance[index]+= balance[right];
                count+=(balance[right]*distance);
            }
            else{
                count+=(abs(balance[index])*distance);
                balance[index]+= balance[right];
